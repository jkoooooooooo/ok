...
import { LegacyFlight, Booking } from '@/types/flight';
...
const BookFlight = () => {
  const [flight, setFlight] = useState<LegacyFlight | null>(null);
  ...
  const bookingMutation = useMutation({
    mutationFn: async (bookingData: { flight_id: string; passenger_name: string; email: string }) => {
      // Create the booking
      ...
      // Update flight seats
      if (flight) {
        await flightService.updateFlightSeats(flight.id, flight.seatsAvailable - 1);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://74dm44apv9.execute-api.eu-north-1.amazonaws.com/flight', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to send to DynamoDB');
      }
  
      console.log('✅ Sent to DynamoDB');
    } catch (err) {
      console.error('❌ Error saving to DynamoDB:', err);
      toast({
        title: "Server Warning",
        description: "Could not sync booking to DynamoDB.",
        variant: "destructive",
      });
    }
  
    // Step 2: Call internal mutation
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <Button 
                    ...
                    disabled={bookingMutation.isPending || !passengerName || !email}
                  >
                    {bookingMutation.isPending ? 'Processing Booking...' : `Book Flight - $${flight.price}`}
                  </Button>
                </form>
              </CardContent>
            </Card>
