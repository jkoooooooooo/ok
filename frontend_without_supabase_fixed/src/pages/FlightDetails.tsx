...
import { LegacyFlight } from '@/types/flight';
...
const FlightDetails = () => {
  ...
  const { data: flight, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const dbFlight = await flightService.getFlightById(flightId);
      return dbFlight ? mapFlightToLegacy(dbFlight) : null;
    },
    enabled: !!flightId,
    onError: (error) => {
  ...
  useEffect(() => {
    if (error || (flight === null && !isLoading)) {
      navigate('/');
    }
  }, [flight, error, isLoading, navigate]);
