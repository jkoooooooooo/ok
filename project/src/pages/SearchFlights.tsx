...
import { SearchParams, LegacyFlight } from '@/types/flight';
import { flightService } from '@/services/flightService';
import { mapFlightToLegacy } from '@/utils/flightMapper';
import { Plane, MapPin } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';

const SearchFlights = () => {
  const [searchResults, setSearchResults] = useState<LegacyFlight[]>([]);
  ...
  const handleSearch = async (params: SearchParams) => {
    ...
    try {
      const flights = await flightService.searchFlights(params.fromCity, params.toCity);
      const legacyFlights = flights.map(mapFlightToLegacy);
      setSearchResults(legacyFlights);
      
      if (legacyFlights.length === 0) {
        toast({
          title: "No flights found",
          description: "Try adjusting your search criteria or check other dates.",
        });
      }
    } catch (error) {
  ...
  const handleBookFlight = (flight: LegacyFlight) => {
